...
class GPSdata {
    ...
    GpsTest(ros::NodeHandle nh_) : n(nh_) {
        ...
        :gps_sub{n.subscribe("/NavSatFix", 100, &GpsTest::gpsCallback)}
    }
   ...
   private:
    ...
    ROS_INFO("Received Message: ", msg->data.c_str());
}
};
...
int main(int argc, char **argv) {
    ...
    gps_node.gps_sub = gps_node.nh.subscribe("/gps/fix", 10, gpsCallback);
    ...
    GpsTest *test_ptr = new GpsTest(nh_);
    ...
    while(ros::ok()) {
      ...
      gpsLong = test_ptr->gpsMsg.longitude;
      gpsLat = test_ptr->gpsMsg.latitude;

      gps_transform(GpsTest);
