    ...
    auto callback = [&](const ros::TimerEvent& event){
        while(target_it != path.end()){
            if(my_bot.target_ahead(*target_it)){
                //This will publish angular twist to rotate the bot facing next waypoint
                my_bot.face_towards(*target_it, pub);
                //This will publich linear velocity part of twists to move the bot towards next waypoint.
                my_bot.step(pub);
            }
            else{
                target_it++;
